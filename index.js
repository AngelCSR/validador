 ...
 document.getElementById("validatorForm").addEventListener("submit", function (e) {
            ...
            const btnHTML = document.activeElement.id === "validarHTML";
            const btnCSS = document.activeElement.id === "validarCSS";

            if (btnHTML) {// Se usa encodeURIComponent para manejar caracteres especiales.
                window.open(`https://validator.w3.org/nu/?doc=${encodeURIComponent(url)}`, "_blank");
            } else if (btnCSS) {
                window.open(`https://jigsaw.w3.org/css-validator/validator?uri=${encodeURIComponent(url)}`, "_blank");
            }
        });

/*

        document.getElementById('validatorForm').addEventListener('click', function() {
            openValidator('html');
        });
        ...
        function openValidator(type) {
            ...
            if (!url || url.indexOf('http') !== 0) {
                alert('Por favor, introduce una URL vÃ¡lida (incluyendo https://).');
                return;
            }
